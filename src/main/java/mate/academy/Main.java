...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        CinemaHall berlinCinemaHall = cinemaHallService.add(cinemaHall);
        CinemaHall secondCinemaHall = new CinemaHall();
        secondCinemaHall.setDescription("Antwerp cinema hall");
        secondCinemaHall.setCapacity(500);
        CinemaHall antwerpCinemaHall = cinemaHallService.add(secondCinemaHall);
        System.out.println(cinemaHallService.get(2L));
        System.out.println(cinemaHallService.getAll());

        MovieSession movieSession = new MovieSession();
        movieSession.setMovie(fastAndFurious);
        movieSession.setCinemaHall(berlinCinemaHall);
        movieSession.setShowTime(LocalDateTime.now());
        MovieSession fastAndFuriosMorningMovieSession = movieSessionService.add(movieSession);
        movieSession.setCinemaHall(antwerpCinemaHall);
        movieSession.setShowTime(LocalDateTime.now().plusHours(4));
        MovieSession fastAndFuriosEveningMovieSession = movieSessionService.add(movieSession);
