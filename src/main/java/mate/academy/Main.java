package mate.academy;

import java.time.LocalDate;
import java.time.LocalDateTime;
import java.util.List;
import mate.academy.lib.Injector;
import mate.academy.model.CinemaHall;
import mate.academy.model.Movie;
import mate.academy.model.MovieSession;
import mate.academy.service.CinemaHallService;
import mate.academy.service.MovieService;
import mate.academy.service.MovieSessionService;

public class Main {
    private static final Injector injector = Injector.getInstance("mate.academy");

    public static void main(String[] args) {
        MovieService movieService = (MovieService) injector.getInstance(MovieService.class);
        Movie fastAndFurious = new Movie("Fast and Furious");
        fastAndFurious.setDescription("An action film about street racing, heists, and spies.");
        movieService.add(fastAndFurious);
        System.out.println(movieService.get(fastAndFurious.getId()));
        Movie transformers = new Movie("Transformers");
        transformers.setDescription("about robots");
        movieService.add(transformers);
        System.out.println(movieService.get(transformers.getId()));
        movieService.getAll().forEach(System.out::println);

        CinemaHallService cinemaHallService =
                (CinemaHallService) injector.getInstance(CinemaHallService.class);
        CinemaHall cinemaHallRed = new CinemaHall();
        cinemaHallRed.setCapacity(100);
        cinemaHallRed.setDescription("Red");
        cinemaHallService.add(cinemaHallRed);
        System.out.println(cinemaHallService.get(cinemaHallRed.getId()));
        CinemaHall cinemaHallWhite = new CinemaHall();
        cinemaHallWhite.setCapacity(200);
        cinemaHallWhite.setDescription("White");
        cinemaHallService.add(cinemaHallWhite);
        System.out.println(cinemaHallService.get(cinemaHallWhite.getId()));
        cinemaHallService.getAll().forEach(System.out::println);

        MovieSession movieSession1 = new MovieSession();
        movieSession1.setMovie(fastAndFurious);
        movieSession1.setCinemaHall(cinemaHallRed);
        movieSession1.setShowTime(LocalDateTime.of(2020,1,10,22,00,00));
        MovieSessionService movieSessionService =
                (MovieSessionService) injector.getInstance(MovieSessionService.class);
        movieSessionService.add(movieSession1);
        MovieSession movieSession2 = new MovieSession();
        movieSession2.setMovie(fastAndFurious);
        movieSession2.setCinemaHall(cinemaHallWhite);
        movieSession2.setShowTime(LocalDateTime.of(2020,1,10,12,00,00));
        movieSessionService.add(movieSession2);
        MovieSession movieSession3 = new MovieSession();
        movieSession3.setMovie(fastAndFurious);
        movieSession3.setCinemaHall(cinemaHallWhite);
        movieSession3.setShowTime(LocalDateTime.of(2020,1,9,12,00,00));
        movieSessionService.add(movieSession3);
        System.out.println(movieSessionService.get(movieSession1.getId()));
        List<MovieSession> availableMovieSessions =
                movieSessionService.findAvailableSessions(fastAndFurious.getId(),
                        LocalDate.of(2020, 1, 10));
        availableMovieSessions.forEach(System.out::println);
    }
}
