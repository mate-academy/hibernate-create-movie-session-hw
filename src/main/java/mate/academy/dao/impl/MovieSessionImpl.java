...
public class MovieSessionImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getFindAvailableSessionsQuery =
                    session.createQuery("from MovieSession where movie.id = :id "
                            + "and showTime between :startOfDay and :endOfDay", MovieSession.class);
            getFindAvailableSessionsQuery.setParameter("id", movieId);
            getFindAvailableSessionsQuery.setParameter("startOfDay", LocalTime.MIN.atDate(date));
            getFindAvailableSessionsQuery.setParameter("endOfDay", LocalTime.MAX.atDate(date));
