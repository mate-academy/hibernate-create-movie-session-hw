...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> availableMovieSessionQuery
                    = session.createQuery("FROM MovieSession m "
                            + "WHERE m.movie.id = :movieId AND DATE(m.showTime) = :date",
                    MovieSession.class);
            availableMovieSessionQuery.setParameter("movieId", movieId);
            availableMovieSessionQuery.setParameter("date", date);
            return availableMovieSessionQuery.getResultList();
        } catch (Exception e) {
            throw new DataProcessingException("Can not get available movie sessions by movie id = "
                    + movieId + ", and date is " + date, e);
        }
    }
}
