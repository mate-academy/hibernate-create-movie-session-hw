        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getSessionsQuery = session.createQuery(
                    "SELECT m FROM MovieSession m WHERE m.movie.id = :movieId "
                                    + "AND m.showTime between :dayStart AND :dayEnd",
                             MovieSession.class);
            getSessionsQuery.setParameter("movieId", movieId);
            getSessionsQuery.setParameter("dayStart", date.atTime(LocalTime.MIN));
            getSessionsQuery.setParameter("dayEnd", date.atTime(LocalTime.MAX));
            return getSessionsQuery.getResultList();
        } catch (Exception e) {
            throw new RuntimeException("Failed to get the list of all movies ", e);
        }
    }
}
