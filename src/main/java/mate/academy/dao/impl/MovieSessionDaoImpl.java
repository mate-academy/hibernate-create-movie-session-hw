        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getAllAvailableSessionsQuery =
                    session.createQuery("from MovieSession where movie.id = :movieId "
                            + "and showTime BETWEEN :startDateTime AND :endDateTime",
                            MovieSession.class);
            getAllAvailableSessionsQuery.setParameter("movieId",movieId);
            getAllAvailableSessionsQuery.setParameter("startDateTime", date.atStartOfDay());
            getAllAvailableSessionsQuery.setParameter("endDateTime", date.atTime(LocalTime.MAX));
