...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = sessionFactory.openSession()) {
            Query<MovieSession> findMovieSessionQuery = session.createQuery("FROM MovieSession ms "
                    + "WHERE ms.movie.id = :id AND ms.showTime >= :startDayTime "
                    + "AND ms.showTime <= :endDayTime", MovieSession.class);
            findMovieSessionQuery.setParameter("id", movieId);
            findMovieSessionQuery.setParameter("startDayTime", date.atStartOfDay());
