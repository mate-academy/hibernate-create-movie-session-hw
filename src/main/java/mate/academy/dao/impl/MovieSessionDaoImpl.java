        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> findAvailableSessionsQuery =
                    session.createQuery("FROM MovieSession ms WHERE ms.movie.id = :id "
                            + "AND ms.showTime BETWEEN :minTime AND :maxTime", MovieSession.class);
            findAvailableSessionsQuery.setParameter("id", movieId);
            findAvailableSessionsQuery.setParameter("minTime",
                    LocalDateTime.of(date, LocalTime.MIN));
            findAvailableSessionsQuery.setParameter("maxTime",
