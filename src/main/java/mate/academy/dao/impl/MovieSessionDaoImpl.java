        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getAllMovieSessionQuery =
                    session.createQuery("FROM MovieSession m "
                                    + "WHERE m.id = :id "
                                    + "AND DATE(m.showTime) = :date",
                            MovieSession.class);
            getAllMovieSessionQuery.setParameter("id", movieId);
            getAllMovieSessionQuery.setParameter("date", java.sql.Date.valueOf(date));
            return getAllMovieSessionQuery.getResultList();
        } catch (Exception e) {
            throw new DataProcessingException("Can't find a session by movie id: " + movieId
