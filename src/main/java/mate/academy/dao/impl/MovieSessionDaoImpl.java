    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = sessionFactory.openSession()) {
            Query<MovieSession> getAllMovieSessionsQuery =
                    session.createQuery("FROM MovieSession m WHERE m.id = :id"
                                    + " AND m.showTime BETWEEN : start AND : end",
                            MovieSession.class);
            getAllMovieSessionsQuery.setParameter("id", movieId);
            getAllMovieSessionsQuery.setParameter("start", LocalDateTime.of(date, LocalTime.MIN));
            getAllMovieSessionsQuery.setParameter("end", LocalDateTime.of(date, LocalTime.MAX));
