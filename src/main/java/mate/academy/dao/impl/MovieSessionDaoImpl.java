        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            Query<MovieSession> getAvailableSessionsQuery =
                    session.createQuery("FROM MovieSession ms "
                            ...
                            + "AND DATE(ms.showTime) = :sqlDate", MovieSession.class);
            getAvailableSessionsQuery.setParameter("movieId", movieId);
            getAvailableSessionsQuery.setParameter("sqlDate", sqlDate);
            return getAvailableSessionsQuery.getResultList();
        } catch (RuntimeException e) {
            throw new RuntimeException("Can't get the list of movie sessions "
