...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId,LocalDate localDate) {
        LocalDateTime fromLocalDateTime = localDate.atTime(LocalTime.MIN);
        LocalDateTime toLocalDateTime = localDate.atTime(LocalTime.MAX);
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> query = session.createQuery("FROM MovieSession ms "
                            + "WHERE ms.movie.id = :movieId"
                            + " AND ms.showTime between :fromLocalDateTime AND :toLocalDateTime",
                    MovieSession.class);
