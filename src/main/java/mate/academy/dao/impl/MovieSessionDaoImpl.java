...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            getMovieSessionQuery.setParameter("startTime", date.atStartOfDay());
            getMovieSessionQuery.setParameter("endTime", date.atTime(23, 59, 59));
            return getMovieSessionQuery.getResultList();
        } catch (Exception e) {
            throw new DataProcessingException("Can't get list of movie session", e);
        }
    }
}
