        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getAvailableSessionsQuery = session
                    .createQuery("FROM MovieSession ms "
                                    + "WHERE ms.showTime between :fromDate AND :toDate "
            ...
            throw new DataProcessingException("Can't get any available movie's session by id:"
                    + movieId + " with this date:" + date, e);
        }
    }
}
