    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        ...
        try (Session session = sessionFactory.openSession()) {
            Query<MovieSession> query = session.createQuery("FROM MovieSession "
                    + "WHERE movie.id = :movieId AND showTime BETWEEN :time1 AND :time2",
                    MovieSession.class);
            query.setParameter("movieId", movieId);
            query.setParameter("time1", LocalDateTime.of(date,
                    LocalTime.of(0, 0)));
            query.setParameter("time2", LocalDateTime.of(date,
                    LocalTime.of(23, 59, 59)));
            return query.getResultList();
        } catch (Exception ex) {
            throw new DataProcessingException("Can't get all entities from table movie_sessions",
                    ex);
        }
    }
}
