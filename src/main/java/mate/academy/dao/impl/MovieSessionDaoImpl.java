...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            getAvailableSessionsQuery.setParameter("start_time",
                    LocalDateTime.of(date, LocalTime.MIN));
            getAvailableSessionsQuery.setParameter("end_time",
                    LocalDateTime.of(date, LocalTime.MAX));
            return getAvailableSessionsQuery.getResultList();
        } catch (Exception e) {
            throw new DataProcessingException("Can't find all available "
                    + "sessions for movie by id: " + movieId + " on date: " + date, e);
        }
    }
}
