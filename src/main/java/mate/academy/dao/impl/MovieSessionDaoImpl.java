        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> movieSessionQuery =
                    session.createQuery("from MovieSession where movie_id = :movieId "
                            + "and show_time between :dateBegin and :dateEnd", MovieSession.class);
            movieSessionQuery.setParameter("movieId",movieId);
            movieSessionQuery.setParameter("dateBegin",date.atTime(LocalTime.MIN));
            movieSessionQuery.setParameter("dateEnd",date.atTime(LocalTime.MAX));
