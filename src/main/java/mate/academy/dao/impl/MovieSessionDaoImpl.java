...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public MovieSession add(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't add new movieSession to DB "
                    + movieSession, e);
        } finally {
            if (session != null) {
                session.close();
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> movieSessionQuery =
                    session.createQuery("from MovieSession ms where ms.movie.id =: movieId "
                            + "and ms.showTime between : dateMin "
                            + "and : dateMax", MovieSession.class);
            movieSessionQuery.setParameter("movieId", movieId);
            movieSessionQuery.setParameter("dateMin", date.atTime(LocalTime.MIN));
            movieSessionQuery.setParameter("dateMax", date.atTime(LocalTime.MAX));
