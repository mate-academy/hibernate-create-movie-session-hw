    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getAvailableSessions =
                    session.createQuery("FROM MovieSession m where m.movie.id = :value "
                                    + "and  m.showTime between :start and :end", MovieSession.class)
                            .setParameter("value", movieId)
                            .setParameter("start", date.atStartOfDay())
                            .setParameter("end", date.atTime(23,59,59));
            return getAvailableSessions.getResultList();
        } catch (Exception e) {
            throw new DataProcessingException("Can't get list of available sessions with movieId: "
                    + movieId + " and on the date: " + date + " from BD.", e);
        }
    }
}
