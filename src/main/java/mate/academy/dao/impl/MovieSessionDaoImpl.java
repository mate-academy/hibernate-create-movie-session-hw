...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long moveId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> getMoviesQuery =
                    session.createQuery("from MovieSession ms WHERE ms.id = :id "
                            + "AND ms.showTime BETWEEN :from AND :to", MovieSession.class);
            getMoviesQuery.setParameter("id", moveId);
            getMoviesQuery.setParameter("from",date.atTime(LocalTime.MIN));
            getMoviesQuery.setParameter("to",date.atTime(LocalTime.MAX));
            return getMoviesQuery.getResultList();
        } catch (Exception e) {
