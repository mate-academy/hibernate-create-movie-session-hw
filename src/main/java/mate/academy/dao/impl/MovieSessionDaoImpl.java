...
public class MovieSessionDaoImpl implements MovieSessionDao {
    ...
    public MovieSession add(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't insert the movie session: " + movieSession, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<MovieSession> query = session.createQuery("SELECT ms FROM MovieSession ms "
                    + "WHERE ms.id = :id AND ms.showTime BETWEEN :fromTime AND :toTime");
            query.setParameter("id", movieId);
            query.setParameter("fromTime", date.atStartOfDay());
            query.setParameter("toTime", date.atTime(23, 59, 59));
